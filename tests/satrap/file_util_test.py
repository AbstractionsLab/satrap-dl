...
class TestFileUtils(unittest.TestCase):
    ...
    @classmethod
    def setUpClass(cls):
        cls.test_file = tu.get_test_data_path("test-file.json")
    ...
    def test_create_local_filename(self):
        ...
        data_folder = tu.TEST_DATA_FOLDER + os.sep
        created_name = file_utils.create_local_filename(data_folder, self.url)
        expected = f"{data_folder}enterprise-attack_{now}.json"
    ...
    def test_validate_file_access(self):
        ...
        invalid_folder = tu.get_test_data_path("new_folder/test.txt")
        with self.assertRaises(ValueError) as err:
            file_utils.validate_file_access(invalid_folder, write=False)
        self.assertIn("new_folder' does not exist",
                      str(err.exception))

    def test_validate_and_create(self):
        new_dir = tu.get_test_data_path("new_folder")
