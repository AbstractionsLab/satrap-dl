...
* All remaining scalar / list fields → YAML frontmatter (alphabetically sorted,
  matching the style of TSS-001.md).
...
If no folders are given the script defaults to the seven YAML-format spec
folders:
    docs/specs/arc  docs/specs/mrs  docs/specs/srs  docs/specs/swd
    docs/specs/tra  docs/specs/trb  docs/specs/tst
...
def _frontmatter_yaml(data: dict) -> str:
    ...
    Keys are sorted alphabetically (matching the TSS/TRS convention).
