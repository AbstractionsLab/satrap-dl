...
To benefit from the analysis functionality of SATRAP that enables automated reasoning, we recommend its [use in Visual Studio Code](/docs/manual/satrap/installation.md#satrap-analysis-platform-in-vs-code). Once the project is open, follow these simple steps.
...
Alternatively, set the URL of a STIX 2.1 data source in the `extract_src` parameter of the [configuration file](/docs/manual/satrap/setup.md) to ingest data from other sources.

**Execute sample functions using the Python CTIAnalysisToolbox**:

Select and run one of our [sample notebooks](/docs/manual/satrap/playbooks.md) to learn how to use the functions of SATRAP and how to apply them on a CTI investigation case.
